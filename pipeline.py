...
def _extract():
    ...
    for news_site_uid in news_sites_uids:
        ...
        subprocess.run(['find','.','-name','{}*'.format(news_site_uid),'-exec','mv','{}','../transform/{}.csv'.format(news_site_uid),';'],cwd='./extract')

def _transform():
    logger.info('Starting transform process')
    for news_site_uid in news_sites_uids:
        dirty_data_filename = '{}.csv'.format(news_site_uid)
        clean_data_filename = 'clean{}'.format(dirty_data_filename)
